...
void FixedSizePool::PoolInformaton()
{
    PrintBorder(INFO_TOTAL);
    PrintTitleAlignCenter("内存池信息",INFO_TOTAL);
    std::cout<<std::left<<std::setw(INFO_HEAD)<<"[已分配页]";
    std::cout<<std::left<<std::setw(INFO_DATA)
    <<this->pages<<std::endl;

    std::cout<<std::left<<std::setw(INFO_HEAD)<<"[每页块数]";
    std::cout<<std::left<<std::setw(INFO_DATA)
    <<this->blocks<<std::endl;

    std::cout<<std::left<<std::setw(INFO_HEAD)<<"[每块大小]";
    std::cout<<std::left<<std::setw(INFO_DATA)
    <<this->block_size<<"Bytes"<<std::endl;

    std::cout<<std::left<<std::setw(INFO_HEAD)<<"[分配总大小]";
    std::cout<<std::left<<std::setw(INFO_DATA)
    <<(this->pages)*(this->blocks)*(this->block_size)<<"Bytes"<<std::endl;

    std::cout<<std::left<<std::setw(INFO_HEAD)<<"[可用总大小]";
    std::cout<<std::left<<std::setw(INFO_DATA)
